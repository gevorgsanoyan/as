...
namespace ASFront.ModelsView
{
    public class BalanceViewModel
    {
        ...
        public BalanceViewModel(Balance item) : this()
        {
            this.Id = item.Id;
            ...
            this.TotalCurrentLiabilities = (item.ShortTermLoans + item.AccountPayable + item.OtherCurrentLiabilities).ToString("N0");
            ...
            this.TotalNonCurrentLiabilitiues =  (item.MediumLongTermLoans + item.OtherNonCurrentLiabilitiues).ToString("N0");
            ...
            double TotalCurrentAssetsNum = (item.Cash + item.BankAccount + item.AccountReceivable + item.Inventory + item.OtherCurrentAssets);
            double TotalNonCurrentAssetsNum = (item.Equipments + item.Vehicles + item.PropertyPlantes + item.OtherNonCurrentAssets);
            double TotalCurrentLiabilitiesNum = (item.ShortTermLoans + item.AccountPayable + item.OtherCurrentLiabilities);
                
            double TotalNonCurrentLiabilitiuesNum =(item.MediumLongTermLoans + item.OtherNonCurrentLiabilitiues); 

            OwnCapitalNum = ((TotalCurrentAssetsNum + TotalNonCurrentAssetsNum) - (TotalCurrentLiabilitiesNum + TotalNonCurrentLiabilitiuesNum));


            double CreditLoadAcraNum = CommonFunction.CalculateCreditLoadAcra(item.applicationId);




            if (OwnCapitalNum != 0)
                LiabilityCapitalNum = (TotalCurrentLiabilitiesNum + TotalNonCurrentLiabilitiuesNum) / OwnCapitalNum;

            if (TotalCurrentLiabilitiesNum != 0)
                CurrentExperienceNum = TotalCurrentAssetsNum / TotalCurrentLiabilitiesNum;



            this.OwnCapital = OwnCapitalNum.ToString("N0");
            this.LiabilityCapital = LiabilityCapitalNum.ToString("N2");
            this.CurrentExperience = CurrentExperienceNum.ToString("N2");


            this.CreditLoadAcra = CreditLoadAcraNum.ToString("N0");
