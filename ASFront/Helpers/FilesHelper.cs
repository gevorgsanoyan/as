...
namespace jQuery_File_Upload.MVC5.Helpers
{
    public class FilesHelper
    {
        ...
        public void UploadAndShowResults(HttpContextBase ContentBase, List<ViewDataUploadFilesResult> resultList, string dirName)
        {
            var httpRequest = ContentBase.Request;
            System.Diagnostics.Debug.WriteLine(Directory.Exists(tempPath));

            String fullPath = Path.Combine(StorageRoot, dirName);
            ...
            foreach (String inputTagName in httpRequest.Files)
            {
                ...
                if (string.IsNullOrEmpty(headers["X-File-Name"]))
                {
                    ...
                    UploadWholeFile(ContentBase, resultList,   dirName);
                }
                else
                {

                    UploadPartialFile(headers["X-File-Name"], ContentBase, resultList,   dirName);
                }
            }
        }


        private void UploadWholeFile(HttpContextBase requestContext, List<ViewDataUploadFilesResult> statuses, string dirName)
        {
            ...
            for (int i = 0; i < request.Files.Count; i++)
            {
                ...
                var fullPath = Path.Combine(pathOnServer, dirName,Path.GetFileName(file.FileName));
        ...
        private void UploadPartialFile(string fileName, HttpContextBase requestContext, List<ViewDataUploadFilesResult> statuses, string dirName)
        {
            var request = requestContext.Request;
            ...
            var fullName = Path.Combine(patchOnServer, dirName,Path.GetFileName(file.FileName));
